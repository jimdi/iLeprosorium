...
- (void)scrollViewPressed:(UIGestureRecognizer *)gestureRecognizer
{
    if (gestureRecognizer.state == UIGestureRecognizerStateBegan && !self.isHighlighted && self.shouldHighlight)
    {
		...
		if ([self.delegate respondsToSelector:@selector(didHighlightSwipeableTableViewCell:)]) {
			[self.delegate didHighlightSwipeableTableViewCell:self];
		}
    }
    ...
    else if (gestureRecognizer.state == UIGestureRecognizerStateEnded)
    {
		...
		if ([self.delegate respondsToSelector:@selector(didUnhighlightSwipeableTableViewCell:)]) {
			[self.delegate didUnhighlightSwipeableTableViewCell:self];
		}
    }
...
- (void)setScrollContentOffset:(CGPoint)offset {
	[self.cellScrollView setContentOffset:offset];
}
